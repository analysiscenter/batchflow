...
class TestBaseset:
    """ Class to hold tests for Baseset. """
...
class TestDatasetIndexBase:
    """ Class to hold tests for DatasetIndex. """
    ...
    def test_shuffle_callable(self, dsindex_list):
        left = dsindex_list._shuffle(shuffle=(lambda _: np.arange(len(dsindex_list))))
        assert set(left) == set(np.arange(len(dsindex_list)))
...
class TestDatasetIndexBatch:
    ...
    def test_next_batch_drop_last(self, dsindex_list):
        ...
        for _ in range(500):
            n_b = dsindex_list.next_batch(batch_size=3,
                                          n_epochs=None,
                                          drop_last=True)
            assert len(n_b) == 3
    ...
    def test_next_batch_small(self, dsindex_big):
        ...
        for _ in range(500):
            n_b = dsindex_big.next_batch(batch_size=len(dsindex_big)-200,
                                         n_epochs=None)
            assert len(n_b) == len(dsindex_big)-200
    ...
    @pytest.mark.xfail(reason='inherits _shuffle flaws(see test_shuffle_callable)')
    def test_next_batch_shuffle_callable(self, dsindex_list):
        dsindex_list.reset_iter()
        n_b = dsindex_list.next_batch(batch_size=5,
                                      shuffle=np.random.permutation)
        assert set(n_b.index) in set(range(2*SIZE, 4*SIZE))
