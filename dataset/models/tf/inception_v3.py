    ...
    ** Configuration **
    ...
    def block(cls, inputs, filters, layout='cna', name='block', **kwargs):
        ...
        with tf.variable_scope(name):
            ...
            branch_3 = conv_block(inputs, [filters[1], filters[0]], [1, 3], layout*2, name='conv_3', **kwargs)

            branch_3_3 = conv_block(inputs, [filters[0]]+[filters[2]]*2, [1, 3, 3], layout*3, name='conv_3_3', **kwargs)

            branch_pool = conv_block(inputs, filters[3], 1, 'p'+layout, 'c_pool', **{**kwargs, 'pool_strides': 1})

            axis = cls.channels_axis(kwargs['data_format'])
            output = tf.concat([branch_1, branch_3, branch_3_3, branch_pool], axis, name='output')
        return output
    ...
    def reduction_block(cls, inputs, filters, layout='cna', name='reduction_block', **kwargs):
        ...
        with tf.variable_scope(name):
            ...
            branch_1_3_3 = conv_block(branch_1_3, filters[2], 3, layout, name='conv_1_3_3', strides=2, padding='valid', **kwargs)

            branch_pool = conv_block(inputs, layout='p', pool_size=3, pool_strides=2, name='max_pooling', padding='valid', **kwargs)
    ...
    def mixed_block(cls, inputs, filters, layout='cna', name='mixed_block', **kwargs):
        ...
        with tf.variable_scope(name):
            branch_1 = conv_block(inputs, filters[0], 1, layout, name='conv_1', **kwargs)
            branch_1_3 = conv_block(branch_1, filters[1], 3, layout, name='conv_1_3', strides=2, padding='valid', **kwargs)

            branch_1_7 = conv_block(inputs, filters[0], [1, [1, 7], [7, 1]], layout*3, name='conv_1_7', **kwargs)
            branch_1_7_3 = conv_block(branch_1_7, filters[0], 3, layout, name='conv_1_7_3', strides=2, padding='valid', **kwargs)
    ...
    def factor_block(cls, inputs, filters, layout='cna', name='factor_block', **kwargs):
        ...
        with tf.variable_scope(name):
            ...
            branch_3 = conv_block(inputs, [filters[1]]*2+[filters[0]] , kernel_size, layout*3, name='conv_3', **kwargs)

            kernel_size = [1, *factor*2]
            branch_7 = conv_block(inputs, [filters[1]]*4+[filters[0]], kernel_size, layout*5, name='conv_7', **kwargs)

            branch_pool = conv_block(inputs, filters[0], 1, 'p'+layout, name='c_pool', **{**kwargs, 'pool_strides': 1})

            axis = cls.channels_axis(kwargs['data_format'])
            output = tf.concat([branch_1, branch_3, branch_7, branch_pool], axis, name='output')
        return output
    ...
    def expanded_block(cls, inputs, filters, layout='cna', name='expanded_block', **kwargs):
        ...
        with tf.variable_scope(name):
            ...
            branch_1_1_3 = conv_block(inputs, filters[1], 1, layout, name='conv_1_1_3', **kwargs)
            branch_3_1 = conv_block(branch_1_1_3, filters[1], [3, 1], layout, name='conv_1_31_3', **kwargs)
            branch_1_3 = conv_block(branch_1_1_3, filters[1], [1, 3], layout, name='conv_1_13_3', **kwargs)
            branch_3_3 = tf.concat([branch_1_3, branch_3_1], axis)

            branch_1_1_5 = conv_block(inputs, filters[2], 1, layout, name='conv_1_1_5', **kwargs)
            branch_1_3_5 = conv_block(branch_1_1_5, filters[1], 3, layout, name='conv_1_3_5', **kwargs)
            branch_3_1 = conv_block(branch_1_3_5, filters[1], [3, 1], layout, name='conv_1_31_5', **kwargs)
            branch_1_3 = conv_block(branch_1_3_5, filters[1], [1, 3], layout, name='conv_1_13_5', **kwargs)
            branch_5_5 = tf.concat([branch_1_3, branch_3_1], axis)

            branch_pool = conv_block(inputs, filters[3], 1, layout='p'+layout, name='c_pool', **{**kwargs, 'pool_strides': 1})

            output = tf.concat([branch_1, branch_3_3, branch_5_5, branch_pool], axis, name='output')
        return output
